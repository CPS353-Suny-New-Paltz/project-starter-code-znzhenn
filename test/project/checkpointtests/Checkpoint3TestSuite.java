...
/**
 * Important note if you're experiencing test failures: An entirely
 * annotation-based injection framework for the tests is NOT allowed for
 * checkpoint 3. If you would like to migrate to such a framework for subsequent
 * checkpoints, talk to Katherine about modifying this test
 */
public class Checkpoint3TestSuite {
	...
	public static void initializeAst() throws Exception {
		...
		// there's no built-in way to link to the src classes but exclude them from the
		// root types
	...
	public void checkImplExists(Class<? extends Annotation> apiAnnotation) throws Exception {
		...
		Assertions.assertTrue(
				Utils.loadAllClasses().stream()
						.anyMatch(clazz -> !clazz.isInterface() && interfaceType.isAssignableFrom(clazz)),
				...
				.assertTrue(
						allTestTypes.stream()
								.anyMatch(clazz -> !clazz.isInterface()
										&& interfaceType.isAssignableFrom(clazz.getActualClass())),
	...
	public void checkIntegrationTestStructure() throws Exception {
		...
		Class<?> interfaceType = getApiClass(ProcessAPI.class);
		CtType<?> testDataStoreClass = allTestTypes.stream()
				.filter(clazz -> !clazz.isInterface() && interfaceType.isAssignableFrom(clazz.getActualClass()))
				.findFirst().get();
		...
		Assertions.assertTrue(
				allTestTypes.stream()
						.anyMatch(clazz -> !clazz.getElements(api1Real).isEmpty()
	...
	private Class<?> getApiImplClass(Class<? extends Annotation> apiAnnotation) {
		Class<?> interfaceType = getApiClass(apiAnnotation);
		return allSrcTypes.stream()
				.filter(clazz -> !clazz.isInterface() && interfaceType.isAssignableFrom(clazz.getActualClass()))
				.findFirst().get().getActualClass();
	}
}